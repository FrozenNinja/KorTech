...
class Waitroom:
    ...
    @commands.command(pass_context=True)
    @checks.admin_or_permissions(change_nickname=True)
    async def register(self, ctx):
        ...
        if server.id in self.riceCog and server.id in self.roleset:
            ...
            await self.bot.say("Type **here** to get access to the Update Channel.")
            ...
            if msg.content.lower().strip() == "here":
                try:
                    ...
                    await self.bot.say("You are now Updating!")
